...
#for FA environment and learner and test_env
#from .. import fortattack, ad_hoc, utils, action_policy
import sys
sys.path.append('/home/sxs2038/KAT/Code/FA')
import fortattack
import ad_hoc_mod
import utils
...
#remove this and use fortattack ??
...
def AnswerSetFinder(Expression, AnswerSetFile):
    ...
    if not re.search('[\>\<\=\!]',Expression):
        ...
        #Expression = re.sub('[A-Z0-9][0-9]?', '[a-z0-9_]+(?:\(.+?\))?', Expression)
        #print('expr',Expression)
        literal = re.findall("(?<!-)"+Expression, AnswerSet) 
        #print(re.findall("occurs",AnswerSet)[0])
...
def compareAnsSet(AnswerSetExp, AnswerSetObs):
    ...
    expectation = AnswerSetFinder('holds(F,I)', AnswerSetExp)+AnswerSetFinder('-holds(A,I)', AnswerSetExp)
    observation = AnswerSetFinder('holds(F,I)', AnswerSetObs)+AnswerSetFinder('-holds(A,I)', AnswerSetObs)
...
def relRepresentation(grndLiterals):
  ...
  for groundedLiterals in grndLiterals:  
    #print('gliterals', groundedLiterals)
    ...
    for n, literals in enumerate(groundedLiterals[:-1]):
     ...
     # 5. include the ungrounded flutes/actions in the "AllRelevantFacts" list if not there yet.
        if n == 2:
            ...
            #print('UL check', ungroundedLit)
            #print('AllRel Update', AllRelevantFacts)
    ungroundLiteral.append(groundedLiterals[-1])                
    ungroundedLiterals.append(ungroundLiteral)
    #print('Literals', ungroundedLiterals)
             
  print('Facts', AllRelevantFacts)
...
def relevantInfo(action, answerSetFile, relations, relevantTerms, tim):
    #openning Answer set file
    f = open(answerSetFile,"r")
    answerSet = f.readlines()
    f.close()
    ...
    #print('ActionTerms', actionTerms)
    #print('relTerms', relevantTerms)
    # retrieving all the fluents and constants that contains the relevant terms as argument
    relevantFacts = []
    relevantFact = []
    #Lines = answerSet.readlines()
    for term in actionTerms:
        for line in answerSet:
        	#print('line', line)
        	timestep_query = re.findall(',([0-9]+)', line)
        	#print('time', timestep_query)
        	if term in line and int(timestep_query[-1]) == tim and 'occurs' not in line: #changed 1 to tim
        		#print('line', line)
        		relevantFact.append(line[:-1])
        #relevantFact = re.findall("(?:\{|\s)(\S*"+term+"\S+\),0\))", answerSet)
        #print('relFacts', relevantFact)
...
# Python list definitions for the learner ASP program
...
others_actions = agent_actions + rotate_actions + shoot_actions
exo_move_actions = ['exo_move(' + agent[0] + ',' + str(x) + ',' + str(y) + ')' for x in x_value for y in y_value]
exo_rotate_actions = ['exo_rotate(' + agent[0] + ',' + direction + ')' for direction in direction]
exo_shoot_actions = ['exo_shoot(' + agent[0] + ')' for attacker in attacker]
exogenous_actions = exo_move_actions + exo_rotate_actions + exo_shoot_actions
action = others_actions + exogenous_actions

# Fluents
inertial_f = ['in(' + agent[0] + ',' + str(x) + ',' + str(y) + ')' for x in x_value for y in y_value]
facing_f = ['facing(' + direction + ',' + str(x1) + ',' + str(y1) + ',' + str(x2) + ',' + str(y2) + ')' for direction in direction for x1 in x_value for y1 in y_value for x2 in x_value for y2 in y_value]
holds_f = inertial_f + facing_f

# Predicates
fort_pred = ['fort(' + str(x) + ',' + str(y) + ')' for x in x_value for y in y_value]
next_to_pred = ['next_to(' + str(x1) + ',' + str(y1) + ',' + str(x2) + ',' + str(y2) + ')' for x1 in x_value for y1 in y_value for x2 in x_value for y2 in y_value]
next_dir_pred = ['next_dir(' + direction1 + ',' + direction2 + ')' for direction1 in direction for direction2 in direction]
facing_pred = ['facing(' + direction + ',' + str(x1) + ',' + str(y1) + ',' + str(x2) + ',' + str(y2) + ')' for direction in direction for x1 in x_value for y1 in y_value for x2 in x_value for y2 in y_value]

Relations = ['in', 'face', 'shot', 'distance_gur_to_learn', 'in_range', 'reached_fort', 'agent_in', 'agent_face', 'agent_shot']
Sorts = [['R', agent], ['A', attacker], ['G', guard]]
missing_axioms = ['holds(shot(A),I+1) :- occurs(shoot(R,A),I).']
'''
...
falsePositive = []
falsePositiveV = []
#setting up environment for simulations
args = get_args()
...
for chck in range(20):#running actions randomly

###   Run different actions in different initial states ...

	#copyfile('pre_with.sp', 'pre_without.sp')#come back here and change!!!!!!!!!!

	stop2 = 0
	ep_count = 0#count episodes to reset environment as in KAT
	...
	while stop2 < 10:
		...
		#print('here maybe?')
		stop1 = 0
		#This loop is to get the unexpected action, we already know it will be a shoot one
		'''Why this loop? Is it needed?
		while not unexpected and not extra and stop1 < 100:
		    ...
		    print('act should be grounded', action)
		    ...
		    planner('learner.sp', actionGround, initialState, 'AnswerSetExp')
		...
		relevantTerms = []
		unexpect_lit = []
		'''Again is it needed? Confirmed not needed
		while count_unexp < 5:
			objects = Objects[:]
			places = Places[:]
			initialState, actionGround = randomState_Action(action, objects, places)
		...
		while count_unexp < train_amount and count_extra < train_amount:  
		    ...
		    #initialState, actionGround = randomState_Action(action, objects, places)
		    #action = random.choice(Actions)
		    #action = 'occurs('+action+',0).'
		    #set action in the learner itself
		    #print('action',action)
		    #actionGround = action
		    #other agent actions
		    print('~~~~~~~~', count_unexp, '~~~~~~~~~~', count_extra,'~~~~~~~~~~', stop2, '~~~~~~',chck)
		    actions_list = act.get_actions(obs)
		    agent_actions = np.array(actions_list).reshape(-1)
		    #print('here')
		    if len(learner_actions) == 0:#calling the learner2 functon to wrte to files
                	learner_actions_exp,actionGround, answer_list = ad_hoc_mod.learner2(obs, alive_ag, previous_action, step, 'pre_with.sp', 'AnswerSetExp', [], 'with.sp', False)#normal actions
                	shoot_goal = False
                	time_killed = 1
                	...
                	with open('AnswerSetObs', "w") as f:
                		for line in answer_list:
                			f.write(f"{line}\n")
                	learner_actions = learner_actions_exp
                			
                				
                	#learner_actions,actiontemp, alist_noneed = ad_hoc_mod.learner2(obs, alive_ag, previous_action, step, 'pre_without.sp', 'AnswerSetObs', [], 'without.sp', False) #preASPLearner_file, answer_file, action, asp, asp_learn
                	
                	#learner_call.append(step)
		    #planner('ASP_agent_oracle.sp', actionGround, initialState, 'AnswerSetObs')
		    #planner('agent_learner.sp', actionGround, initialState, 'AnswerSetExp')
		    print('grounded action', actionGround)
		    #update for next run
		    ...
		    if True:#actionGround != '':
		    	# finding the relevant facts 
		    	relevantFacts, relevantTerms = relevantInfo(actionGround, 'AnswerSetExp', Relations, relevantTerms, time_killed)    
		    	print('rel Facts', relevantFacts)
		...
		if count_unexp == train_amount:
		    ...
		    if len(unexpect_lit) > 1:
		    	...
		    	keys_list = list(unexpect_lit.keys())
		    	max_index = tot_vars.index(max(tot_vars))
		    	unexp_literal = keys_list[max_index]
		    else:
		    	#unexp_literal = unexpect_lit.keys()[0]
		    	unexp_literal  = list(unexpect_lit.keys())[0]
		    print('causal law to info', causal_law)
		    print('ul to info', unexp_literal)
		    info = ['causal_law', [unexp_literal[1:]], [causal_law[-1][1][0].replace('.','')], (len(AllRelevantFacts) - 1)]
		    print('info', info)
	...
	# loading data from robot simulation
		...
		#writer('without.sp', 'axioms_'+str(level)+'.txt')
