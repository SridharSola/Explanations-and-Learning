...
def learner(obs, alive_ag, previous_actions, step_nu):
    if obs[0][0] == 1.0: # ad hoc agent alive
        ...
        input, hist,next_terms, facing_terms, goal_term, goal_guard, learner_grid, learner_dir = get_actions_terms(obs, alive_ag, previous_actions,)
        ...
        actions = process_answerlist(answer_list1, learner_dir, learner_grid)
        ######################################################
        print('stepnu', step_nu)
        print('answerlist', answer_list)
        answer_split = ((answer_list.rstrip().split('\n\n'))[0]).strip('{}\n').split(', ')
        answer_list = [re.sub(r'(\d+)\)$', lambda match: f'{int(match.group(1)) + step_nu})', item) if re.search(r'(\d+)\)$', item) else item for item in answer_split]
        acts = []
        for f in answer_list:
        	#hist.append(f)
        	if 'occurs' in f:#action
        		acts.append(f)
        	else:
        		hist.append(f)
        answer_list = acts
        ...
        print('answerlist2', answer_list)
        for f in answer_list:
        	hist.append(f)
        with open("answers", "a") as f:
        	for line in answer_list:
        		f.write(f"{line}\n")
        history = [re.sub(r'(\d+)\).$', lambda match: f'{int(match.group(1)) + step_nu})', item) if re.search(r'(\d+)\).$', item) else item for item in hist]

        with open("history", "a") as f:
        	for line in history:
        		f.write(f"{line}\n")
        #print('history', history)
        print('gterm', goal_term)
        hist.append(goal_term[0])
        
    else:
        actions = [0,0,0]
    return actions

#for axiom learning, reason with a partcular asp file
def learner2(obs, alive_ag, previous_actions, step_nu, preASPLearner_file, answer_file, action, asp_learn, without = True):
    if obs[0][0] == 1.0: # ad hoc agent alive
        reader = open(preASPLearner_file, 'r')
        print('pre', preASPLearner_file)
        pre_asp = reader.read()
        reader.close()
        pre_asp_split = pre_asp.split('\n')
        display_marker_index = pre_asp_split.index(display_marker)
        input, hist,next_terms, facing_terms, goal_term, goal_guard, learner_grid, learner_dir, in_range = get_actions_terms(obs, alive_ag, previous_actions, without)
        print('goal', goal_term)
        '''
        if 'shot' in goal_term[0] and in_range:
        	action = ['occurs(shoot(learner,'+re.findall('attacker[0-9]', goal_term[0])[0]+'),1).']
        elif 'shot' in goal_term[0] and not in_range: #do nothing
        	print('not in range')
        	action = []
        else:
        	action = []
        '''
        #print('action', action)
        answer_list = runASPLearner2(obs, input, next_terms, facing_terms, goal_term, goal_guard, pre_asp_split, display_marker_index, answer_file, action, asp_learn)
        answer_list1 = answer_list
        #print('al1', answer_list1)
        action_list = []
        if True:#'shot' in goal_term[0]:#len(answer_list) != 0:
        	#print('shot goal')
        	answer_split = ((answer_list.rstrip().split('\n\n'))[0]).strip('{}\n').split(', ')
        	...
        	'''
        action = ''
        if 'shot' in goal_term[0]:
        	for f in action_list:
        		if 'shoot' in f:
        			action = f
        			break
        elif 'shot' not in goal_term[0] and len(action_list) != 0:
        	for f in action_list:
        		if 'move' in f:
        			action = f
        			break
...
def get_actions_terms(obs, alive_ag, previous_actions, without):
    ...
    for timestep in range(2): # consider done?
        ...
        if timestep != 0:
        	sim = False#True #set belief sort as simulation after writing the original environment observations
        #print('kkkkkkkkkkk')
        ...
        #input, hist, learner_grid, learner_dir, learner_pos, guards_info, attackers_info, x_min, x_max, y_min, y_max = get_terms(obs, timestep, input, hist, sim)
        input, hist, learner_grid, learner_dir, learner_pos, guards_info, attackers_info, x_min, x_max, y_min, y_max, in_range = get_terms2(obs, timestep, input, hist, sim)
        obs, done, alive_ag, info = test_env.step(future_actions)
    next_terms, facing_terms, goal_term, goal_guard = get_additional_terms(learner_pos,learner_grid,guards_info,attackers_info,x_min,x_max,y_min,y_max, without)
    return input, hist, next_terms, facing_terms, goal_term, goal_guard, learner_grid, learner_dir, in_range
...
def process_answerlist(answer_list, learner_dir, learner_grid):
    ...
    if len(actions) > 3: # change depend on the results.....................only 3 actions!
        actions = actions[:3]
    return actions
...
def runASPLearner(obs, input, next_terms, facing_terms, goal_term, goal_guard, pre_asp_split, display_marker_index):
    ...
    print('asplearn', answer_split)
    try:
    	with open("asf", "a") as asf:
...
# return answer sets for the new ASP file for learner2
def runASPLearner2(obs, input, next_terms, facing_terms, goal_term, goal_guard, pre_asp_split, display_marker_index, answer_file, action, asp_learn):
    if goal_guard:#write action as well
        asp_split = const_term + pre_asp_split[:display_marker_index] + next_terms + facing_terms + goal_term + input + action + pre_asp_split[display_marker_index:]
    else:
        asp_split = const_term + pre_asp_split[:display_marker_index] + next_dir_terms + next_terms + facing_terms + goal_term + input + action + pre_asp_split[display_marker_index:]
    asp = '\n'.join(asp_split)
    print('asp_learn', asp_learn)
    f1 = open(asp_learn, 'w')
    #print('obs', obs)
    #print('asp_split',asp_split)
    f1.write(asp)
    f1.close()
    start = time.time()
    answer = subprocess.check_output('java -jar ASP/sparc.jar ' +asp_learn+' -A',shell=True)
    end = time.time()
    answer_split = (answer.decode('ascii'))
...
def get_terms(obs, step, input, hist, sim = False):
    x_min = None
    x_max = None
    y_min = None
    y_max = None
    ...
    for agent in range(len(obs)):
        ...
        if agent == 0: # ad hoc agent
            ...
            learner_grid_cur = get_gridno(obs[agent][1], obs[agent][2])
            #A = get_tri_pts_arr(obs[agent][1], obs[agent][2], obs[agent][3])
            #Keep appending to input the same for reasoning. Change hist for explanations with may_hold
            ...
            if sim == False:
            	#hist.append('holds(in('+agent_name+','+str(grid[0])+','+str(grid[1])+'),'+str(step)+').')
            	hist.append('holds(face('+agent_name+','+direction+'),'+str(step)+').')
            	hist.append('-holds(shot('+agent_name+'),'+str(step)+').')
            else:#use may_hold predicate instead
            	hist.append('may_hold(in('+agent_name+','+str(grid[0])+','+str(grid[1])+'),'+str(step)+').')
            	hist.append('may_hold(face('+agent_name+','+direction+'),'+str(step)+').')
            	hist.append('-may_hold(shot('+agent_name+'),'+str(step)+').')
        else:
            ...
            if sim == False:
            	hist.append('holds(agent_in('+agent_name+','+str(grid[0])+','+str(grid[1])+'),'+str(step)+').')
            	hist.append('holds(agent_face('+agent_name+','+direction+'),'+str(step)+').')
            else:
            	hist.append('may_hold(agent_in('+agent_name+','+str(grid[0])+','+str(grid[1])+'),'+str(step)+').')
            	hist.append('may_hold(agent_face('+agent_name+','+direction+'),'+str(step)+').')
            if obs[agent][0] == 1.0:#not shot yet
                input.append('-holds(shot('+agent_name+'),'+str(step)+').')
                if sim == False:
                	hist.append('-holds(agent_shot('+agent_name+'),'+str(step)+').')
                else:
                	hist.append('-may_hold(agent_shot('+agent_name+'),'+str(step)+').')
                if in_range_of_agent(learner_grid, grid, learner_dir, direction) and 'attacker' in agent_name:#if attckr in range
                	#print('here as in range')
                	if sim == False:
                		hist.append('holds(in_range(learner,'+agent_name+'),'+str(step)+').')
                	else:
                		hist.append('may_hold(in_range(learner,'+agent_name+'),'+str(step)+').')
                elif in_range_of_agent(learner_grid, grid, learner_dir, direction) == False and 'attacker' in agent_name:
                	#print('not in range')
                	if sim == False:
                		hist.append('-holds(in_range(learner,'+agent_name+'),'+str(step)+').')
                	else:
    ...
    #print('getterms iinput', input)
    
    return input,hist,learner_grid,learner_dir,learner_pos,guards_info,attackers_info,x_min,x_max,y_min,y_max
    
# process obs and set the fluents that need to be included and considered in the new ASP program
# ALEADY same loop is running THREE times inside this code. NEED TO OPTIMIZE.
def get_terms2(obs, step, input, hist, sim = False):
    x_min = None
    x_max = None
    y_min = None
    y_max = None
    ...
    return input,hist,learner_grid,learner_dir,learner_pos,guards_info,attackers_info,x_min,x_max,y_min,y_max, in_range

# get goal, next_to and facing terms
def get_additional_terms(learner_pos,learner_grid,guards_info,attackers_info,x_min,x_max,y_min,y_max,without):
    goal_term,x_min,x_max,y_min,y_max,goal_guard = get_goal(learner_pos,learner_grid,guards_info,attackers_info,x_min,x_max,y_min,y_max,without)
...
def get_goal(learner_pos,learner_grid,guards_info,attackers_info,x_min,x_max,y_min,y_max,without):
    ...
    for attacker in range(len(attackers_info)):
        if without:
        	if attackers_info[attacker][5] > 6:#6: # attacker in danger zone need not be alive
        	...
        	if attackers_info[attacker][1] == 1.0 and attackers_info[attacker][5] > 6:#6: # attacker in danger zone
            		arr = np.array([float(attackers_info[attacker][2]),float(attackers_info[attacker][3])])
            		attackers_info[attacker].append(np.sqrt(np.sum(np.square(arr-learner))))
            		attackers_info[attacker].append(np.sqrt(np.sum(np.square(arr-door))))
            		danger_attackers.append(attackers_info[attacker])
...
def in_range_of_agent(L_pos, A_pos, L_facing, A_facing):
    XL, YL = L_pos
    XA, YA = A_pos
    #print('XL', XL, 'YL', YL, 'XA', XA, 'YA', YA)
    #print('L_facing', L_facing)
    if L_facing == "south":
        return (6 <= YL - YA <= 7 and XL - 3 <= XA <= XL + 3) or (5 <= YL - YA <= 6 and XL - 2 <= XA <= XL + 2)
    elif L_facing == "north":
        return (6 <= YA - YL <= 7 and XL - 3 <= XA <= XL + 3) or (5 <= YA - YL <= 6 and XL - 2 <= XA <= XL + 2)
    elif L_facing == "west":
        return (6 <= XL - XA <= 7 and YL - 3 <= YA <= YL + 3) or (5 <= XL - XA <= 6 and YL - 2 <= YA <= YL + 2)
    elif L_facing == "east":
        return (6 <= XA - XL <= 7 and YL - 3 <= YA <= YL + 3) or (5 <= XA - XL <= 6 and YL - 2 <= YA <= YL + 2)
    else:
