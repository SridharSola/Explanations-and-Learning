    ...
    if obs[0][0] == 1.0: # ad hoc agent alive
        ...
        with open("answers", "a") as f:
        	for line in answer_list:
        		f.write(f"{line}\n")
        history = hist
        #history = [re.sub(r'(\d+)\).$', lambda match: f'{int(match.group(1)) + step_nu})', item) if re.search(r'(\d+)\).$', item) else item for item in hist]#rewrte history to reflect overall episode step
        history2 = [re.sub(r'(\d+)\).$', lambda match: f'{int(match.group(1)) + step_nu})', item) if re.search(r'(\d+)\).$', item) else item for item in hist2]
...
def learner2(obs, alive_ag, previous_actions, step_nu, preASPLearner_file, answer_file, action, asp_learn):
    if obs[0][0] == 1.0: # ad hoc agent alive
        reader = open(preASPLearner_file, 'r')
...
def get_actions_terms(obs, alive_ag, previous_actions):
    ...
    return input, hist, next_terms, facing_terms, goal_term, goal_guard, learner_grid, learner_dir
...
def runASPLearner2(obs, input, next_terms, facing_terms, goal_term, goal_guard, pre_asp_split, display_marker_index, answer_file, action, asp_learn):
    ...
    try:
    	with open(answer_file, "w") as asf:
        	asf.write(answer_split + '\n')
    except Exception as e:
    	print("An error occurred:", e)
    acts = []
